...
<script setup lang="ts">
    ...
    useNuxtApp().hook("app:notification:show", (data: NotificationData | undefined) => {
        if (!data) {
            logger.debug(`Hiding any present notification...`);
        ...
        if (data.level == NotificationLevel.DEBUG && !import.meta.env.DEV) {
        ...
        logger.debug(`Showing notification ${duration > 0 ? 'for ' + duration + 'ms' : 'indefinitely'}:`, data);
